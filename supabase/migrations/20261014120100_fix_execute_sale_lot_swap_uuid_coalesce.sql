-- Correção aditiva — COALESCE uuid/text em public.execute_sale_lot_swap(p_payload jsonb).
-- Não edita 20261014120000_execute_sale_lot_swap.sql (já aplicada no DEVELOP).
-- Sem mutação de venda/lote/parcela/contrato. Sem backfill. Sem executar a troca.
--
-- Causa no DEVELOP hoynysmynxncdlptuzub:
--   blocks.company_id = uuid
--   blocks.tenant_id  = text
-- O guard de tenant dos lotes misturava esses tipos no COALESCE (uuid + text).
-- sales.company_id e sales.tenant_id permanecem uuid+uuid — intocados.
-- operator_user_id, sale_id, contract ids e IDs de parcela não misturam uuid/text.
