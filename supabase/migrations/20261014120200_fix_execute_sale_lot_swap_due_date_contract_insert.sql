-- Correção aditiva — due_date date/text e INSERT de contrato no schema DEVELOP.
-- Não edita 20261014120000 nem 20261014120100 (já aplicadas no DEVELOP).
-- Sem mutação de venda/lote/parcela/contrato. Sem backfill. Sem executar a troca.
--
-- 1) COALESCE das novas parcelas: ambos os lados date.
-- 2) INSERT de contrato usa só colunas presentes no OpenAPI DEVELOP.
--    regenerated_by, installments, needs_regenerar, pdf_url, sale_value e
--    superseded_by são opcionais via information_schema. Não cria colunas.
-- Regras financeiras/contratuais da Fase 4 permanecem iguais.
...
BEGIN
  ...
  -- blocks.company_id uuid + blocks.tenant_id text: ambos os lados do COALESCE precisam ser uuid.
  IF COALESCE(v_from.company_id, NULLIF(btrim(v_from.tenant_id), '')::uuid) IS DISTINCT FROM v_company_id
     OR COALESCE(v_to.company_id, NULLIF(btrim(v_to.tenant_id), '')::uuid) IS DISTINCT FROM v_company_id THEN
  ...
  IF EXISTS (
    ...
    EXECUTE 'UPDATE public.contracts SET installments = $1 WHERE id = $2'
      USING COALESCE((p_payload->'new_contract'->>'installments')::integer, v_installments),
            v_new_contract_id;
  END IF;
  ...
  IF EXISTS (
    ...
    EXECUTE 'UPDATE public.contracts SET pdf_url = NULL WHERE id = $1'
      USING v_new_contract_id;
  END IF;
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'contracts' AND column_name = 'sale_value'
  ) THEN
    EXECUTE 'UPDATE public.contracts SET sale_value = $1 WHERE id = $2'
      USING v_new_price, v_new_contract_id;
  END IF;
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
