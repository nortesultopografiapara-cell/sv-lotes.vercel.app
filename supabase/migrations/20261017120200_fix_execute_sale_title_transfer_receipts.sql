-- Correção do modelo financeiro da Transferência de titularidade.
-- 20261017120000 e 20261017120100 já aplicadas no DEVELOP hoynysmynxncdlptuzub.
-- CREATE OR REPLACE da função. Sem ALTER/DROP. Sem backfill. Sem tocar em cobrança externa.
--
-- Pagamentos históricos de A permanecem imutáveis (mesmo id e customer_id).
-- Parcelas futuras antigas de A ficam cancelado (sem DELETE).
-- Novas finance_receipts são criadas para B, ligadas ao contrato novo.
-- sales.customer_id e blocks.customer_id continuam passando para B.
-- Cobrança bancária permanece fora desta RPC.
...
BEGIN
  ...
  IF cardinality(v_cancel) > 0 THEN
    SELECT COUNT(*) INTO v_cancel_ok
    FROM public.finance_receipts
    WHERE sale_id = v_transfer.sale_id
      AND id = ANY (v_cancel);
    IF v_cancel_ok IS DISTINCT FROM cardinality(v_cancel) THEN
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION 'TITLE_TRANSFER_EXECUTE:RECEIPT_PAID_IMMUTABLE:Pagamento histórico não pode ser cancelado nem reatribuído.';
    END IF;
  END IF;
  ...
  IF cardinality(v_cancel) > 0 THEN
    ...
    WHERE sale_id = v_transfer.sale_id
      ...
      AND lower(btrim(coalesce(status, ''))) NOT IN (
        'pago', 'paid', 'cancelado', 'canceled', 'cancelled', 'cancelada'
      )
  ...
  LOOP
    ...
    VALUES (
      ...
      v_transfer.sale_id,
      v_to_expected,
      COALESCE(v_transfer.project_id, v_block.project_id),
      v_transfer.block_id,
    ...
    IF EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'finance_receipts' AND column_name = 'contract_id'
    ) THEN
      EXECUTE 'UPDATE public.finance_receipts SET contract_id = $1 WHERE id = $2'
        USING v_new_contract_id, v_rid;
    END IF;
  END LOOP;
  ...
  RETURN jsonb_build_object(
    ...
    'created_receipt_count', v_created_count,
    'canceled_future_count', cardinality(v_cancel)
  );
END;
$$;

COMMENT ON FUNCTION public.execute_sale_title_transfer(jsonb) IS
  'P4/P5 — transferência de titularidade atômica. Preserva pagas de A, cancela futuras antigas, cria parcelas de B. Sem cobrança externa. Sem ReleaseLot. Sem RPC da Troca.';
