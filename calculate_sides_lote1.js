...
// Lote 1 coordinates
const pts = [
  [-49.881184272946, -6.18453980693245],
  [-49.8812170681049, -6.1845876855483],
  [-49.8812850483594, -6.18458362673846],
  [-49.8812662117385, -6.18426762953877],
  [-49.8811373990494, -6.18427532039917]
];

const utmPts = pts.map(p => wgs84ToUtm(p[0], p[1], 22));

console.log("=== LOTE 1 UTM ZONE 22S ENDPOINTS ===");
...
for (let i = 0; i < n; i++) {
  const ptCurrent = utmPts[i];
  const ptNext = utmPts[(i + 1) % n];
  console.log(`Segment ${i+1} -> ${(i+1)%n + 1 || n}: ${dist(ptCurrent, ptNext).toFixed(3)}`);
}
