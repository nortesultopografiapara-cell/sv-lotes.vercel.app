...
function title(text: string): string {
  return `<h3 class="estrela-clause-title" style="font-family: 'Times New Roman', Times, serif; font-size: 12pt; font-weight: bold; text-align: center; text-transform: uppercase; margin: 18px 0 10px 0;">${escEstrelaHtml(text)}</h3>`;
}
...
export function buildEstrelaDoSulClausesHtml(
  ...
  const objectTable = `
    <table class="estrela-table" style="width:100%; border-collapse:collapse; font-size:11pt; margin:8px 0 14px 0;">
      <tr>
        <th style="border:1px solid #111; padding:4px 6px; text-align:left; width:32%;">Informação</th>
        <th style="border:1px solid #111; padding:4px 6px; text-align:left;">Detalhamento</th>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">Nome do Projeto</td>
        <td style="border:1px solid #111; padding:4px 6px;">${escEstrelaHtml(ctx.enterpriseName)}</td>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">Localização do Imóvel</td>
        <td style="border:1px solid #111; padding:4px 6px;">${escEstrelaHtml(ctx.enterpriseLocation)}</td>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">Área Vendida</td>
        <td style="border:1px solid #111; padding:4px 6px;">${areaCell || '—'}</td>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">Confrontações</td>
        <td style="border:1px solid #111; padding:4px 6px;">${escEstrelaHtml(ctx.confrontacoesText) || '—'}</td>
      </tr>
      ${
        ctx.partnershipNote
          ? `<tr>
        <td style="border:1px solid #111; padding:4px 6px;">Outras informações</td>
        <td style="border:1px solid #111; padding:4px 6px;">${escEstrelaHtml(ctx.partnershipNote)}</td>
      </tr>`
          : ''
      }
    </table>`;
  ...
  const financeTable = `
    <table class="estrela-table estrela-finance-table" style="width:100%; border-collapse:collapse; font-size:11pt; margin:8px 0 14px 0;">
      <tr>
        <th style="border:1px solid #111; padding:4px 6px; text-align:left; width:42%;">ITEM</th>
        <th style="border:1px solid #111; padding:4px 6px; text-align:left;">VALOR / DETALHAMENTO</th>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">VALOR TOTAL DO IMÓVEL</td>
        <td style="border:1px solid #111; padding:4px 6px;">${money(ctx.valorTotalFmt, ctx.valorTotalExtenso)}</td>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">VALOR DE CORRETAGEM</td>
        <td style="border:1px solid #111; padding:4px 6px;">${money(ctx.valorCorretagemFmt, ctx.valorCorretagemExtenso)}</td>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">VALOR DO SINAL/ENTRADA (ARRAS)</td>
        <td style="border:1px solid #111; padding:4px 6px;">${money(ctx.valorSinalFmt, ctx.valorSinalExtenso)}</td>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">PARCELAS E VALORES</td>
        <td style="border:1px solid #111; padding:4px 6px;">${escEstrelaHtml(ctx.parcelasResumo)}</td>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">VENCIMENTO DA 1ª PARCELA</td>
        <td style="border:1px solid #111; padding:4px 6px;">${escEstrelaHtml(ctx.dataPrimeiraParcelaFmt || '—')}</td>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">ÍNDICE DE CORREÇÃO ANUAL</td>
        <td style="border:1px solid #111; padding:4px 6px;">${escEstrelaHtml(ctx.indiceCorrecaoCapa)}</td>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">MULTA MORATÓRIA POR ATRASO</td>
        <td style="border:1px solid #111; padding:4px 6px;">${ESTRELA_LATE_FINE_PERCENT}% (dois por cento) sobre a parcela vencida</td>
      </tr>
      <tr>
        <td style="border:1px solid #111; padding:4px 6px;">JUROS DE MORA POR ATRASO</td>
        <td style="border:1px solid #111; padding:4px 6px;">${ESTRELA_LATE_INTEREST_PERCENT}% (um por cento) ao mês</td>
      </tr>
    </table>`;
  ...
  return `
    ...
    <div class="contract-clause">
      ...
      <p style="font-size:9pt; margin: 8px 0 0 0;"><sup>5</sup> Natureza jurídica: as ARRAS nos termos dos arts. 417 a 420 do Código Civil – É considerado um valor em dinheiro entregue pelas partes compradoras ao momento da assinatura de um contrato com objetivo de garantia de cumprimento do negócio e, em outras oportunidades, podendo ser aplicado como indenização pré-fixada.</p>
      <p style="font-size:9pt; margin: 4px 0 12px 0;"><sup>6</sup> A comissão de corretagem possui natureza de remuneração pelos serviços de intermediação e não será restituída em caso de distrato, sendo este valor na importância de ${escEstrelaHtml(ctx.valorCorretagemFmt)}.</p>
    </div>
    ...
    <div class="contract-clause">
      ...
      ${p('<strong>3.3.</strong> O COMPRADOR reconhece e declara, de forma irretratável, ter procedido à prévia e minuciosa vistoria física da unidade rural ora transacionada, bem como atesta ter verificado sua inscrição no loteamento, sua topografia, demarcação perimetral e marcos divisórios, em cotejo com o memorial descritivo e a plantilha do empreendimento.')}
    ...
    <div class="contract-clause">
      ...
      ${p('<strong>8.1.</strong> Da Anuência e Requisitos para a Cessão de Direitos: A cessão, transferência ou alienação dos direitos e obrigações decorrentes deste instrumento a terceiros (Cessionários) somente será admitida mediante o cumprimento estrito e cumulativo dos seguintes requisitos:<br/>I. Estar o COMPRADOR (Cedente) rigorosamente em dia com o pagamento de todas as parcelas, tributos e taxas referentes ao imóvel;<br/>II. Solicitação formal e por escrito apresentada pelo COMPRADOR com antecedência mínima de 30 (trinta) dias;<br/>III. Aprovação em prévia e rigorosa análise cadastral e de capacidade financeira do pretenso Cessionário, a critério exclusivo da VENDEDORA;<br/>IV. Emissão de aprovação expressa e formal por parte da VENDEDORA;<br/>V. Pagamento, à vista, de uma Taxa de Cessão e Anuência fixada no patamar de ${ESTRELA_ASSIGNMENT_FEE_PERCENT}% (cinco por cento) do valor total e atualizado deste contrato, destinada ao custeio de despesas administrativas, jurídicas e de refação de cadastro.')}
    ...
    <div class="contract-clause">
      ...
      ${p('<strong>12.6.</strong> O COMPRADOR autoriza a VENDEDORA a coletar e tratar seus dados pessoais estritamente para fins de gestão contratual, emissão de boletos, cobrança e órgãos de proteção ao crédito, em conformidade com a Lei nº 13.700/2018 (LGPD).')}
