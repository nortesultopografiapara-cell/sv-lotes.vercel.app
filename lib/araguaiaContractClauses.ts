...
export function buildAraguaiaClausesHtml(ctx: AraguaiaContractContext): string {
  ...
  return `
    ...
    ${clauseHtml(
      ...
      ${itemP(
        `<strong>2</strong> – O chacreamento é fruto da área pertencente à matrícula nº <strong>55.278</strong>, lançada no Livro 02 do Registro Geral, ficha 01, do 1º Ofício de Registro de Imóveis de Parauapebas – PA, correspondente aos lotes 33; 34 e 36 – parte 01 do Assentamento Palmares Sul – Parauapebas – PA, cabendo ${B.to} a responsabilidade pela documentação necessária conforme estabelece a legislação regente, inclusive os serviços de <strong>georreferenciamento</strong>, arcando com todos os custos incidentes;`,
      )}
      ${keepTogether(
    ...
    ${clauseHtml(
      ...
      ${keepTogether(
        ...
        itemP(
          `<strong>C</strong> – ${B.The} se ${B.declara} ${ciente} de que adquiriu o imóvel com a infraestrutura descrita na cláusula nona deste contrato, sendo que quaisquer outros serviços ou melhoramentos públicos que vierem a ser exigidos pelos poderes públicos correrão à suas expensas.`,
        ),
      )}
      `,
    )}
