...
export async function validatePortalLotSaleScope(
...
): Promise<PortalValidatedSaleScope | null> {
  ...
  if (!saleId || !customerId || !scopeCompanyId) return null;
  ...
  if (String(sale.customer_id) !== customerId) return null;
  ...
  if (!effectiveCompanyId || effectiveCompanyId !== scopeCompanyId) return null;

  return {
    saleId,
    customerId,
    companyId: effectiveCompanyId,
    contractId: scope.contractId ? String(scope.contractId) : null,
    sale: {
      id: String(sale.id),
...
export function assertPortalContractBelongsToSale(
  ...
  if (String(contract.sale_id || '') !== validated.saleId) return false;
  if (validated.contractId && String(contract.id) !== validated.contractId) return false;
  return true;
}
