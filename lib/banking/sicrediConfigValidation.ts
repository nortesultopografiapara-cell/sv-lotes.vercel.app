...
/** Snapshot mínimo para validar integração Sicredi (Fase 2.0-Sicredi — sem API real). */
export type SicrediConfigValidationInput = {
  clientId?: string;
  clientSecret?: string;
...
export type SicrediConfigValidationResult = {
  ok: boolean;
...
/** Valida campos obrigatórios Sicredi — não chama API externa. */
export function validateSicrediConfig(
  input: SicrediConfigValidationInput,
  ...
  if (missingFields.length === 0) {
    return {
      ...
      missingFields: [],
      message:
        'Configuração Sicredi válida. API real ainda não habilitada nesta fase (Fase 2.0-Sicredi).',
    };
  }
  ...
  return {
    ok: false,
    missingFields,
    message: `Campos obrigatórios Sicredi ausentes: ${labels.join(', ')}.`,
  };
}

export function sicrediValidationInputFromIntegration(
  integration: {
    clientId?: string;
    environment?: string;
    agency?: string;
