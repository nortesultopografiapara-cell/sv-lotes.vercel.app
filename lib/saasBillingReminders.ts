...
async function insertSaasBillingReminderAudit(
  ...
  await supabaseAdmin.from('audit_logs').insert({
    ...
    company_id: input.companyId,
    module: 'SAAS_BILLING',
    action: input.ok ? 'SAAS_BILLING_REMINDER_EMAIL' : 'SAAS_BILLING_REMINDER_EMAIL_FAILED',
...
export async function runSaasBillingReminders(
  ...
  for (const candidate of candidates) {
    ...
    for (const reminderType of reminderTypes) {
      result.processed += 1;
      try {
        const item = await processSaasBillingReminderForCharge(
          supabaseAdmin,
          candidate,
          reminderType,
...
export async function getSaasBillingReminderStats(
  ...
  return SAAS_BILLING_REMINDER_DEFINITIONS.map((definition) => {
    const rows = (data || []).filter((row) => row.reminder_type === definition.type);
    const last = rows[0];
    return {
