...
export function buildSaleContractRepresentativeSignatureHtml(params: {
  ...
  if (cpf) {
    return `<p style="margin: 2px 0 0 0; font-size: 8pt; line-height: 1.25;">${name}<br/>Representante legal<br/>CPF: ${cpf}${suffix}</p>`;
  }

  const companyLabel = company || 'IMOBILIÁRIA';
  return `<p style="margin: 2px 0 0 0; font-size: 8pt; line-height: 1.25;">${name}<br/>Representante legal da ${companyLabel.toUpperCase()}${suffix}</p>`;
}
