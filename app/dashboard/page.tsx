    ...
    async function loadDashboardStats() {
      ...
      try {
        ...
        if (validProjectIds.length > 0) {
          ...
          if (!error && data) {
            data.forEach((lot) => {
              if (selectedProjectId && lot.project_id !== selectedProjectId)
                return;
    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-8">
      ...
      <div className="mb-6 flex items-center gap-4">
        <label
          ...
          className="text-[var(--color-text-muted)] text-sm font-medium"
        >
          Projeto:
        </label>
        <select
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
        <div className="xl:col-span-2 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl overflow-hidden flex flex-col shadow-lg h-[400px] md:h-[500px]">
          <div className="p-5 flex justify-between items-center border-b border-[var(--color-border)] bg-[var(--color-surface)] z-10">
            ...
            <Link
              href="/map"
              className="flex items-center gap-2 text-sm font-medium text-[var(--color-text-muted)] hover:text-white transition-colors bg-[var(--color-background)] px-3 py-1.5 rounded-lg border border-[var(--color-border)]"
            >
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between gap-8 text-[13px]">
                  <div className="flex items-center gap-3 text-white">
                    <div className="w-3.5 h-3.5 rounded-[4px] bg-[var(--color-success)]" />{" "}
                    Disponível
                  </div>
                ...
                <div className="flex items-center justify-between gap-8 text-[13px]">
                  <div className="flex items-center gap-3 text-white">
                    <div className="w-3.5 h-3.5 rounded-[4px] bg-[var(--color-warning)]" />{" "}
                    Reservado
                  </div>
                  <span className="font-mono text-white text-right">
                ...
                <div className="flex items-center justify-between gap-8 text-[13px]">
                  <div className="flex items-center gap-3 text-white">
                    <div className="w-3.5 h-3.5 rounded-[4px] bg-[var(--color-danger)]" />{" "}
                    Vendido
                  </div>
                  <span className="font-mono text-white text-right">
                    {loading ? "-" : stats.sold}
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-6 shadow-lg mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <SummaryItem
            ...
            value={
              loading ? "-" : stats.available + stats.reserved + stats.sold
            }
            icon={MapIcon}
            color="bg-[var(--color-success)]/10 text-[var(--color-success)]"
          />
    ...
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] p-5 md:p-6 rounded-2xl relative overflow-hidden group hover:border-[var(--color-border-hover)] transition-colors shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-2">
          <div
            className={`p-2.5 rounded-xl block md:hidden self-start w-fit ${iconColor}`}
          >
            <Icon className="w-6 h-6" strokeWidth={1.5} />
          </div>
          <p className="text-[13px] text-[var(--color-text-muted)] font-medium leading-tight md:mt-2">
            {title}
          </p>
        ...
        <div
          className={`p-3.5 rounded-2xl hidden md:flex items-center justify-center ${iconColor} shadow-lg`}
        >
...
function SummaryItem({ title, value, icon: Icon, color }: any) {
  return (
    <div className="flex items-center gap-4">
      ...
      <div>
        <p className="text-[13px] text-[var(--color-text-muted)] mb-0.5">
          {title}
        </p>
  ...
  return (
    <div className="flex gap-4 items-start group cursor-pointer">
      ...
      <div className="flex-1 min-w-0 flex flex-col md:flex-row md:items-center justify-between gap-1">
        ...
        <p className="text-[12px] text-[var(--color-text-muted)] md:text-right flex-shrink-0">
          {time}
        </p>
      </div>
    </div>
