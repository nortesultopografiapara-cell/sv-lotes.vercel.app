...
export default function MasterProfilePage() {
  ...
  if (!user || user.role !== 'SUPER_ADMIN') {
    return (
       <div className="flex-1 p-8 text-center bg-[#0b1111] flex items-center justify-center flex-col">
          <Shield className="w-16 h-16 text-gray-700 mb-4" />
          <h2 className="text-xl font-bold text-white mb-2">Acesso Negado</h2>
          <p className="text-gray-400">Esta área é restrita para contas Super Admin.</p>
       </div>
    );
  }
    ...
    <div className="sv-page sv-page--scroll-y p-4 md:p-8 bg-[#0b1111]">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <header className="mb-8">
          ...
          <p className="text-gray-400 text-sm">Gerencie suas configurações de segurança e credenciais avançadas.</p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Security Config */}
          <div className="bg-[#151a23] border border-[#1f232b] rounded-2xl p-6 shadow-xl">
             ...
             <div className="space-y-4 mb-6">
                <div className="p-3 bg-[#0b1111] rounded-lg border border-[#2d3340]">
                   <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-1">Conta Master</p>
                   <p className="text-sm font-medium text-white">{user.name}</p>
                </div>
                <div className="p-3 bg-[#0b1111] rounded-lg border border-[#2d3340]">
                   <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-1">E-mail Principal</p>
                   <p className="text-sm font-medium text-white">{user.email}</p>
                </div>
             </div>

             <form onSubmit={handlePasswordChange} className="space-y-4 pt-6 border-t border-[#1f232b]">
          ...
          <div className="space-y-6">
            ...
            {/* 2FA */}
            <div className="bg-[#151a23] border border-[#1f232b] rounded-2xl p-6 shadow-xl relative overflow-hidden">
               ...
               <p className="text-sm text-gray-400 mb-4 line-clamp-2">Adicione uma camada extra de segurança utilizando Google Authenticator ou Authy na conta admin.</p>
               <button disabled className="w-full py-2 rounded-lg bg-[#202530] text-gray-500 font-medium text-sm cursor-not-allowed">
            ...
            <div className="bg-[#151a23] border border-[#1f232b] rounded-2xl p-6 shadow-xl">
               ...
               <div className="space-y-3 mb-5">
                  <div className="flex items-center gap-3 p-3 bg-[#0b1111] rounded-lg border border-green-500/20">
                     <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                     <div className="flex-1">
                        <p className="text-xs font-semibold text-gray-200">Sessão Atual (Você)</p>
                        <p className="text-[11px] text-gray-500">IP: 192.168.0.1 • Chrome (MacOS)</p>
                     </div>
                  </div>
                  <div className="flex items-center gap-3 p-3 bg-[#0b1111] rounded-lg border border-[#2d3340]">
                     <span className="w-2 h-2 rounded-full bg-gray-500" />
                     <div className="flex-1">
                        <p className="text-xs font-semibold text-gray-400">Última Sessão</p>
                        <p className="text-[11px] text-gray-500">IP: 172.16.1.4 • Firefox (Windows) • Há 5 horas</p>
                     </div>
                  </div>
               </div>
            ...
            <div className="bg-[#151a23] border border-[#1f232b] rounded-2xl p-6 shadow-xl">
               <h3 className="text-[15px] font-semibold text-white flex items-center gap-2 mb-2">
                  <Activity className="w-4 h-4 text-orange-400" /> Registro de Atividades
               </h3>
               <p className="text-xs text-gray-500 mb-4">Como Super Admin, todas as suas ações vitais no SaaS são registradas.</p>
               
               <button className="flex items-center justify-between w-full p-3 bg-[#0b1111] hover:bg-[#1a1f29] border border-[#2d3340] rounded-lg transition-colors text-left group">
