...
import {
  validateInstallmentImportBuffer,
} from '@/lib/imports/modules/installments/importService';
...
export async function POST(request: Request) {
  try {
    ...
    const context = await loadInstallmentImportContext(auth.ctx.admin, auth.ctx.tenantId);
    const validation = await validateInstallmentImportBuffer(buffer, file.name, context);

    return NextResponse.json({ validation });
  } catch (err) {
    console.error('[data-migration/installments/validate]', err);
