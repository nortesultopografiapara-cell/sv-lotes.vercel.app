...
import { executeBrokerImportBuffer } from '@/lib/imports/modules/brokers/importService';
...
export async function POST(request: Request) {
  try {
    ...
    const result = await executeBrokerImportBuffer({
    ...
    console.error('[data-migration/brokers/execute]', err);
    const message =
      err instanceof Error ? err.message : 'Erro ao importar corretores.';
