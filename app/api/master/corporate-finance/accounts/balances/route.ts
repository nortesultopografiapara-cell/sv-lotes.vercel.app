...
import { computeAccountBalance } from '@/lib/master/corporateFinance/cashMath';
import { listCorporateAccounts } from '@/lib/master/corporateFinance/service';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const a of accounts) {
      const bal = await computeAccountBalance(supabaseAdmin, a.id);
      balances.push({
        ...a,
        balance: bal,
      });
    ...
    const message = err instanceof Error ? err.message : 'Falha ao calcular saldos.';
