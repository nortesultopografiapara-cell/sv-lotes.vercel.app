...
async function run() {
  ...
  for (const contract of contracts) {
      ...
      let receipts_sum = 0;
      if (contract.sale_id) {
         const { data: recs } = await supabase.from("finance_receipts").select("amount").eq("sale_id", contract.sale_id).neq("status", "cancelled");
         if (recs && recs.length) receipts_sum = recs.reduce((a, b) => a + Number(b.amount || 0), 0);
      }
      
      let tenantData = {};
      if (contract.tenant_id) {
          const { data: t } = await supabase.from('companies').select('*').eq('id', contract.tenant_id).maybeSingle();
          if (t) tenantData = t;
      }
      ...
      await supabase.from("contracts").update({ generated_html: newHtml, ...contractPayloadPartial }).eq("id", contract.id);
      updatedCount++;
  }

  console.log("Updated contracts:", updatedCount);
}
