...
const regex = /await supabase\.from\('payments'\)\.insert\(paymentsToInsert\);\s*\}/;

const injection = `await supabase.from('payments').insert(paymentsToInsert);
                }

                // --- NOVO: GERAR CONTRATO E FINANCEIRO AUTOMATIZADO ---
                ...
                if (contractErr) console.warn("Erro ao criar contrato em contracts:", contractErr);
                    ...
                    console.log("Contrato criado com sucesso, ID:", contractInserted?.id);
...
if (regex.test(content)) {
    ...
    console.log("REGEX FAILED. FILE CONTENTS OF THOSE LINES:");
    const idx = content.indexOf("paymentsToInsert");
