...
export function UserProfileModals({ user, company, activeModal, setActiveModal }: { user: any, company: any, activeModal: 'profile' | 'password' | 'security' | null, setActiveModal: (v: any) => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[300] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-[#121318] border border-[var(--color-border)] rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
        
        {/* Header */}
        <div className="px-6 py-4 border-b border-[var(--color-border)] flex justify-between items-center bg-[#151a23]">
          ...
          <button onClick={closeModal} className="text-[var(--color-text-muted)] hover:text-white transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6">
            ...
            <div className="space-y-4">
               ...
               <div className="space-y-3">
                 <div className="flex justify-between items-center py-2 border-b border-[var(--color-border)]/50">
                    <span className="text-[var(--color-text-muted)] text-sm">CNPJ/Doc</span>
                    <span className="text-white text-sm font-medium">{company?.cnpj || company?.document || '-'}</span>
                 </div>
                 <div className="flex justify-between items-center py-2 border-b border-[var(--color-border)]/50">
                    <span className="text-[var(--color-text-muted)] text-sm">Telefone</span>
                    <span className="text-white text-sm font-medium">{company?.phone || '-'}</span>
                 </div>
                 <div className="flex justify-between items-center py-2 border-b border-[var(--color-border)]/50">
                    <span className="text-[var(--color-text-muted)] text-sm">Plano Atual</span>
                    <span className="bg-[var(--color-primary)]/10 text-[var(--color-primary)] px-2 py-0.5 rounded text-xs font-bold border border-[var(--color-primary)]/20">{company?.plan || 'Standard'}</span>
                 </div>
                 <div className="flex justify-between items-center py-2">
                    <span className="text-[var(--color-text-muted)] text-sm">Data de Cadastro</span>
                    <span className="text-white text-sm font-medium">{new Date(user?.created_at).toLocaleDateString('pt-BR')}</span>
                 </div>
               </div>
               
               <div className="mt-6">
                  <p className="text-xs text-center text-[var(--color-text-muted)]">Para editar os dados da empresa, acesse as Configurações Geriais.</p>
               </div>
            </div>
          )}
          ...
          {activeModal === 'password' && (
            <form onSubmit={handlePasswordChange} className="space-y-5">
               ...
               <div>
                  <label className="block text-xs font-medium text-[var(--color-text-muted)] mb-1.5 uppercase tracking-wider">Senha Atual</label>
                  <div className="relative">
                     <input
                       type={showPasswords ? 'text' : 'password'}
                       value={currentPassword}
                       onChange={e => setCurrentPassword(e.target.value)}
                       className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] text-white rounded-lg px-4 py-2.5 focus:outline-none focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary)] transition-all"
                       placeholder="Digite sua senha atual"
                       autoComplete="current-password"
                     />
                     <button type="button" onClick={() => setShowPasswords(!showPasswords)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)] hover:text-white">
               ...
               <div>
                  <label className="block text-xs font-medium text-[var(--color-text-muted)] mb-1.5 uppercase tracking-wider">Nova Senha</label>
                  <input
                    type={showPasswords ? 'text' : 'password'}
                    value={newPassword}
               ...
               <div>
                  <label className="block text-xs font-medium text-[var(--color-text-muted)] mb-1.5 uppercase tracking-wider">Confirmar Nova Senha</label>
                  <input
                    type={showPasswords ? 'text' : 'password'}
               ...
               <div className="flex gap-3 pt-4 border-t border-[var(--color-border)]">
                  <button type="button" onClick={closeModal} className="flex-1 px-4 py-2 bg-transparent border border-[var(--color-border)] hover:bg-[var(--color-surface)] text-gray-300 rounded-lg font-medium transition-colors">
                     Cancelar
                  </button>
            ...
            <div className="space-y-5">
               <div className="p-4 bg-[var(--color-primary)]/5 border border-[var(--color-primary)]/20 rounded-xl flex items-start gap-3">
                 ...
                 <div>
                   ...
                   <p className="text-xs text-[var(--color-text-muted)]">Suas permissões e sessão atual estão ativas e monitoradas.</p>
                 </div>
               </div>
               
               <div className="space-y-3 pt-2">
                 <div className="flex flex-col py-2 border-b border-[var(--color-border)]/50">
                    <span className="text-[var(--color-text-muted)] text-xs uppercase tracking-wider mb-1">Email de Autenticação</span>
                    <span className="text-white text-sm">{sessionInfo?.email || '-'}</span>
                 </div>
                 <div className="flex justify-between items-center py-2 border-b border-[var(--color-border)]/50">
                    <span className="text-[var(--color-text-muted)] text-xs uppercase tracking-wider">Status da Conta</span>
                 ...
                 <div className="flex justify-between items-center py-2 border-b border-[var(--color-border)]/50">
                    <span className="text-[var(--color-text-muted)] text-xs uppercase tracking-wider">Sessão</span>
                    <span className="text-gray-300 text-sm">Dispositivo atual</span>
                 </div>
                 <div className="flex justify-between items-center py-2 border-b border-[var(--color-border)]/50">
                    <span className="text-[var(--color-text-muted)] text-xs uppercase tracking-wider">Último Login</span>
               ...
               <div className="pt-4 space-y-2">
                  <h4 className="text-xs font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-2">Futuros Recursos</h4>
                  <button disabled className="w-full flex items-center justify-between p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg opacity-50 cursor-not-allowed">
                     ...
                     <span className="text-[10px] bg-gray-800 px-1.5 py-0.5 rounded text-[var(--color-text-muted)]">Em breve</span>
                  </button>
                  <button disabled className="w-full flex items-center justify-between p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg opacity-50 cursor-not-allowed">
                     <div className="flex items-center gap-2">
