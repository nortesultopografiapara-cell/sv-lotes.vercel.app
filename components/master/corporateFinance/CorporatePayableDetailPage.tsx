...
function DetailInner() {
  ...
  async function confirmSecureDelete(confirmWord: string) {
    ...
    setDeleteError(null);
    try {
      const res = await fetch(`/api/master/corporate-finance/payables/${row.id}/delete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...bodyAuth(),
      ...
      setDeleteOpen(false);
      setTimeout(() => router.push('/master/corporate-finance/payables'), 900);
    } catch (err) {
  ...
  return (
    <div className={styles.page}>
      ...
      <MasterSecureDeleteModal
        ...
        recordLabel={
          row
            ? `${row.code} — ${row.supplier_name || row.description || 'sem descrição'}`
            : ''
        }
