...
export default function LotDrawer() {
  ...
  useEffect(() => {
     if (isDrawerOpen && selectedLot) {
        setLoading(true);
        // Fetch contract and client info
  ...
  const handleClose = () => {
    ...
    setTimeout(() => setSelectedLot(null), 300); // clear after animation
  };
  ...
  return (
    <>
      <div 
        ...
        onClick={handleClose}
      />
      <div className={`fixed right-0 top-16 bottom-0 w-[400px] bg-[var(--color-surface)] border-l border-[var(--color-border)] z-50 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out transform ${isDrawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        {/* Footer Actions */}
        <div className="p-4 border-t border-[var(--color-border)] bg-[var(--color-surface)] grid grid-cols-2 gap-3">
