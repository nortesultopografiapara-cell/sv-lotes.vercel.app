/**
 * Aplica tabelas de lembretes automáticos ao comprador SOMENTE no DEVELOP.
 * npx tsx scripts/develop/apply-company-buyer-reminders.ts
 */
...
const MIGRATION = '20261024120000_company_buyer_reminders.sql';
...
async function main() {
  ...
  if (!sql.includes('company_buyer_reminder_settings') || !sql.includes('company_buyer_reminder_logs')) {
    throw new Error('ABORT: SQL fora do escopo dos lembretes automáticos ao comprador.');
  }
  ...
  try {
    ...
    const tables = await client.query(`
      select to_regclass('public.company_buyer_reminder_settings') as settings,
             to_regclass('public.company_buyer_reminder_logs') as logs
    `);
    console.log(
      JSON.stringify(
        {
          ok: Boolean(tables.rows[0]?.settings && tables.rows[0]?.logs),
          appliedRef: dbRef,
    ...
    if (!tables.rows[0]?.settings || !tables.rows[0]?.logs) process.exit(1);
  } finally {
    await client.end();
  }
}
