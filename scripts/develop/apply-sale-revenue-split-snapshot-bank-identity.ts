/**
 * Aplica a migration da Fase 2 (freeze bancário no snapshot) SOMENTE no DEVELOP.
 * npx tsx scripts/develop/apply-sale-revenue-split-snapshot-bank-identity.ts
 *
 * Não aplica em Production. Sem backfill. Sem mutação de legs/webhook/emissão.
 */
...
const MIGRATION = '20261019120000_sale_revenue_split_snapshot_bank_identity.sql';
const TABLE = 'sale_revenue_split_snapshot_participants';
...
async function main() {
  ...
  if (
    /ALTER TABLE public\.charge_revenue_split_legs/i.test(sql) ||
    /bank_credentials/i.test(sql) ||
    /seller_parties_json/i.test(sql)
  ) {
    throw new Error('ABORT: SQL fora do escopo da Fase 2.');
  }
  ...
  try {
    ...
    const check = await client.query(`
      ...
      where table_schema = 'public'
        ...
        and column_name in (${EXPECTED_COLUMNS.map((n) => `'${n}'`).join(', ')})
