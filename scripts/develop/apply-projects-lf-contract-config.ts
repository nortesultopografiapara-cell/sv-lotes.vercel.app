/**
 * Aplica projects.lf_contract_config_json SOMENTE no DEVELOP.
 * npx tsx scripts/develop/apply-projects-lf-contract-config.ts
 *
 * Não aplica em Production. Sem backfill. Não grava Antônio.
 */
...
const MIGRATION = '20261026120000_projects_lf_contract_config_json.sql';
...
async function main() {
  ...
  if (!sql.includes(COLUMN)) {
    throw new Error('ABORT: SQL fora do escopo LF Imóveis por empreendimento.');
  }
  ...
  async function verifyViaRest(): Promise<{
    ...
    const probe = await sb.from('projects').select(COLUMN).limit(1);
    if (!probe.error) return { columnPresent: true };
    return { columnPresent: false, error: probe.error.message };
  }

  if (!dbUrl) {
    if (!loaded.service || /SENSITIVE/i.test(loaded.service)) {
      console.log(
        JSON.stringify({
          ok: false,
  ...
  try {
    ...
    const populated = await client.query(`
      ...
      where lf_contract_config_json is not null
    `);
