/**
 * Aplica a migration aditiva da qualificação do Representante Legal SOMENTE no DEVELOP.
 * npx tsx scripts/develop/apply-legal-representative-qualification.ts
 *
...
const MIGRATION = '20261012120000_companies_legal_representative_qualification.sql';
const EXPECTED_COLUMNS = ['contract_legal_rg_uf', 'legal_representative_address'] as const;
  ...
  try {
    ...
    const check = await client.query(`
      ...
      where table_schema = 'public'
        ...
        and column_name in ('contract_legal_rg_uf', 'legal_representative_address')
      order by column_name
    `);
    const names = check.rows.map((r) => String(r.column_name));
    const extra = names.filter((n) => !EXPECTED_COLUMNS.includes(n as (typeof EXPECTED_COLUMNS)[number]));
      ...
      JSON.stringify(
        {
          ...
          appliedRef: dbRef,
          columns: check.rows,
          expectedOnly: EXPECTED_COLUMNS,
          missing,
          unexpectedFromQuery: extra,
        },
        null,
        2,
      ),
