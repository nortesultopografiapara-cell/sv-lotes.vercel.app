/**
 * Aplica a migration aditiva de identificação bancária SOMENTE no DEVELOP.
 * npx tsx scripts/develop/apply-financial-account-bank-identity.ts
 *
 * Não aplica em Production. Sem backfill. Sem mutação de split/webhook.
 */
...
const MIGRATION = '20261018120000_company_financial_accounts_bank_identity.sql';
const TABLE = 'company_financial_accounts';
...
async function main() {
  ...
  if (
    /seller_parties_json/i.test(sql) ||
    /charge_revenue_split_legs/i.test(sql) ||
    /ALTER TABLE public\.bank_credentials/i.test(sql)
  ) {
    throw new Error('ABORT: SQL fora do escopo da Fase 1.');
  }
  ...
  try {
    ...
    const check = await client.query(`
      ...
      where table_schema = 'public'
        ...
        and column_name in ('bank_name', 'bank_code', 'agency', 'account_number', 'account_digit', 'bank_account_kind')
    ...
    console.log(
      JSON.stringify(
        {
          ok: missing.length === 0 && nonNullable.length === 0,
          appliedRef: dbRef,
          columns: check.rows,
          missing,
    ...
    if (missing.length > 0 || nonNullable.length > 0) process.exit(1);
  } finally {
    await client.end();
  }
}
