/**
 * Aplica o guard de assinatura interna do vendedor SOMENTE no DEVELOP.
 * npx tsx scripts/develop/apply-block-client-vendor-signature.ts
 */
...
const MIGRATION = '20261022120000_block_client_vendor_signature.sql';
...
async function main() {
  ...
  if (!sql.includes('contract_signatures_block_client_vendor_signed')) {
    throw new Error('ABORT: SQL fora do escopo da Fase 3.');
  }
  ...
  try {
    ...
    const trig = await client.query(`
      ...
      where tgname in (
        'contract_signature_parties_block_client_vendor_signed',
        'contract_signatures_block_client_vendor_signed'
      )
      order by tgname
    `);
    console.log(
    ...
    if (trig.rows.length !== 2) process.exit(1);
  } finally {
    await client.end();
  }
}
