/**
 * Aplica a migration aditiva de sale_title_transfers SOMENTE no DEVELOP.
 * npx tsx scripts/develop/apply-sale-title-transfers.ts
 *
...
const MIGRATION = '20261016120000_sale_title_transfers.sql';
const TABLE = 'sale_title_transfers';
...
async function main() {
  ...
  if (
    /\bALTER TABLE public\.sale_lot_swaps\b/i.test(sql) ||
    /\bexecute_sale_lot_swap\b/i.test(sql) ||
    /seller_parties_json/i.test(sql)
  ) {
    throw new Error('ABORT: SQL não pode alterar Troca de lote ou Mundo Novo.');
  }
