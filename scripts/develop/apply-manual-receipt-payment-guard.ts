/**
 * Aplica a migration da baixa manual autorizada SOMENTE no DEVELOP.
 * npx tsx scripts/develop/apply-manual-receipt-payment-guard.ts
 */
...
const MIGRATION = '20261021120000_manual_receipt_payment_authorization.sql';
...
async function main() {
  ...
  if (!sql.includes('finance_receipts_block_client_paid_update')) {
  ...
  try {
    ...
    const fn = await client.query(`
      ...
      where proname = 'execute_authorized_manual_receipt_payment'
    `);
    console.log(
      JSON.stringify(
        {
          ok: trig.rows.length === 1 && fn.rows.length === 1,
          appliedRef: dbRef,
    ...
    if (trig.rows.length !== 1 || fn.rows.length !== 1) process.exit(1);
  } finally {
    await client.end();
  }
}
