/**
 * Aplica a correção COALESCE uuid/text de execute_sale_lot_swap SOMENTE no DEVELOP.
 * npx tsx scripts/develop/apply-fix-execute-sale-lot-swap-uuid-coalesce.ts
 *
...
const MIGRATION = '20261014120100_fix_execute_sale_lot_swap_uuid_coalesce.sql';
...
async function main() {
  ...
  if (
    ...
    !sql.includes("COALESCE(v_to.company_id, NULLIF(btrim(v_to.tenant_id), '')::uuid)")
  ) {
    throw new Error('ABORT: correção COALESCE uuid/text dos lotes ausente.');
  }
  if (
    /coalesce\(v_from\.company_id,\s*v_from\.tenant_id\)/i.test(sql) ||
    /coalesce\(v_to\.company_id,\s*v_to\.tenant_id\)/i.test(sql)
  ) {
    throw new Error('ABORT: ainda há COALESCE uuid+text em blocks.tenant_id.');
  }
