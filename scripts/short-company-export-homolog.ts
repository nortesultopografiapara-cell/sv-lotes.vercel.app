/**
 * Homologação curta F0+F1 via APIs Master (sem endpoint temporário).
 *
 * Requer SUPER_ADMIN userId (query/body) — não imprime secrets.
 *
 * Uso:
...
const PREVIEW =
  ...
  'https://sv-lotes-vercel-inppphqsx.vercel.app';
const COMPANY_ID =
...
async function main() {
  if (!USER_ID) {
    console.log(
      JSON.stringify({
        ok: false,
        error: 'SUPER_ADMIN_USER_ID obrigatório (não gravar em arquivo)',
      }),
    );
    process.exit(2);
  }
